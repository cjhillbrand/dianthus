...
use jvm_value::{JvmValue, to_jvm_value};
...
pub fn ldc(thread_id: usize, runtime_data: &mut RunTimeData)
{
	...
	let constants: &Vec<ConstantContainer> = &runtime_data.get_constant_pool(
		current_stack_frame.get_executing_class()
	);

	dbg!(":#?", &constants[cp_index]);

	let jvm_value: (Option<JvmValue>, Option<JvmObject>)  = to_jvm_value(constants, cp_index);
	if !jvm_value.0.is_none()
	{
