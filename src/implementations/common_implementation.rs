...
pub fn load(thread_id: usize, runtime_data: &mut RunTimeData) {
	let stack_frame: &StackFrame = runtime_data.get_current_stack_frame(thread_id);
	let pc: usize = stack_frame.get_pc();
	let index: usize = stack_frame.get_code()[pc + 1] as usize;

	load_n(thread_id, runtime_data, index, 2);
}

pub fn load_0(thread_id: usize, runtime_data: &mut RunTimeData) { load_n(thread_id, runtime_data, 0, 1); }

pub fn load_1(thread_id: usize, runtime_data: &mut RunTimeData) { load_n(thread_id, runtime_data, 1, 1); }

pub fn load_2(thread_id: usize, runtime_data: &mut RunTimeData) { load_n(thread_id, runtime_data, 2, 1); }

pub fn load_3(thread_id: usize, runtime_data: &mut RunTimeData) { load_n(thread_id, runtime_data, 3, 1); }
...
pub fn store(thread_id: usize, runtime_data: &mut RunTimeData) {
	let stack_frame: &StackFrame = runtime_data.get_current_stack_frame(thread_id);
	let pc: usize = stack_frame.get_pc();
	let index: usize = stack_frame.get_code()[pc + 1] as usize;

	store_n(thread_id, runtime_data, index, 2);
}
